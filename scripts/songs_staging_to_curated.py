...
def fetch_songs_from_mysql(songs: list):
    ...
    try:
        with connection.cursor() as cursor:
            formatted_titles = ','.join(['(%s, %s)'] * len(songs))
            query_values = [item for song in songs for item in [song["title"], song["artist"]]]
            
            cursor.execute(f"""
                SELECT s.*, a.name as artist_name, a.bio as artist_bio, a.image_url as artist_image_url 
                FROM songs s 
                JOIN artists a ON s.artist_id = a.id 
                WHERE (s.title, a.name) IN ({formatted_titles})
            """, query_values)
...
def migrate_data_to_mongodb(songs: list):
...
if __name__ == "__main__":
    if len(sys.argv) < 3 or len(sys.argv) % 2 == 0:
        print("Usage : python staging_to_curated.py <title1> <artist1> [<title2> <artist2> ...]")
        sys.exit(1)
    
    songs = [{"title": sys.argv[i], "artist": sys.argv[i + 1]} for i in range(1, len(sys.argv), 2)]
    migrate_data_to_mongodb(songs)
